...
public class AwesomeLinkFilter implements Filter {
    ...
    public List<ResultItem> getResultItemsFile(final String line, final int startPoint) {
        ...
        final String projectDir = projectRootManager.getContentRootUrls()
                .get(0).replace("file://", "")+ "/";
        ...
        for (FileLinkMatch match : matches) {
            ...
            if (match.match.startsWith("./")) {
                ...
                // Make it an absolute path, because that's what's in the fileCache and matchingFiles.
                String absolutePath = relativePath.replace("./", projectDir);

                // The plugin originally listed ALL files that matched the filename
                // (i.e. index.html gave ["c:/project/index.html","c:/project/src/index.html", ...] )
                // So here we filter out files that don't match exactly.
                // I thought it would be easier than changing the fileCache and still have all other code work.
                for (Iterator<VirtualFile> iterator = matchingFiles.iterator(); iterator.hasNext(); ) {
